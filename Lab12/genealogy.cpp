  ...
  void visit(Woman *w) override {
    ...
    if (father != nullptr)
      cout << "Maiden name for " << w->getFirstName()
	   << " is " << father->getLastName() << '\n';
    else
      cout << w->getFirstName() 
	   << " a first generation woman\n";
  }
};
...
public:
  MarriageAdvisor(string oneFirst,
		  string oneLast,
		  string twoFirst,
		  string twoLast):
  ...
  void visit(Man *m) override {
    if (m->getFirstName() == oneFirst_ &&
        m->getLastName() == oneLast_) {
      personOneFound_ = true;
      checkIfCanMarry(m);
    }

    else if (m->getFirstName() == twoFirst_ &&
             m->getLastName() == twoLast_) {
  ...
  void visit(Woman *w) override {
    ...
    if (w->getFirstName() == oneFirst_ &&
	lastNameHolder->getLastName() == oneLast_) {
      personOneFound_ = true;
      checkIfCanMarry(w);
    }

    else if (w->getFirstName() == twoFirst_ &&
	     lastNameHolder->getLastName() == twoLast_) {
      personTwoFound_ = true;
      checkIfCanMarry(w);
    }
  }
  ...
  void checkIfCanMarry(Woman *w) {
    ...
    Man *father = static_cast<Man*>(w->getFather());
    if (father != nullptr) {
      if (oneFound && father->getFirstName() == twoFirst_ &&
	  father->getLastName() == twoLast_) {
	canMarry_ = false;
	return;
      }
      if (!oneFound && father->getFirstName() == oneFirst_ &&
	...
	return;
      ...
      Man *fatherOfFather = static_cast<Man*>
	(father->getFather());
      if (fatherOfFather != nullptr) {
	Woman *motherOfFather = static_cast<Woman*>
	  (father->getFather()->getSpouse());
	if (motherOfFather != nullptr) {
	  for (auto e: motherOfFather->getChildren()) {
	    if (e->getSpouse() == nullptr && fatherOfFather != nullptr) {
	      if (oneFound && fatherOfFather->getLastName() == twoLast_ &&
		  e->getFirstName() == twoFirst_) {
		canMarry_ = false;
		return;
	      }
	      if (!oneFound && fatherOfFather->getLastName() == oneLast_ &&
    ...
    if (mother != nullptr) {
      if (oneFound && mother->getFirstName() == twoFirst_ &&
	  father->getLastName() == twoLast_) {
	canMarry_ = false;
	return;
      }
      if (!oneFound && mother->getFirstName() == oneFirst_ &&
	  father->getLastName() == oneLast_) {
	canMarry_ = false;
	return;
      }
	...
	if (e->getSpouse() == nullptr && father != nullptr) {
	  if (oneFound && father->getLastName() == twoLast_ &&
	      e->getFirstName() == twoFirst_) {
	    canMarry_ = false;
	    return;
	  }
	  if (!oneFound && father->getLastName() == oneLast_ &&
              e->getFirstName() == oneFirst_) {
            canMarry_ = false;
            return;
          }
      ...
      // check for parents spouses siblings
      Man *fatherOfMother = static_cast<Man*>
        (mother->getFather());
      if (fatherOfMother != nullptr) {
	Woman *motherOfMother = static_cast<Woman*>
	  (mother->getFather()->getSpouse());
	if (motherOfMother != nullptr) {
	  for (auto e: motherOfMother->getChildren()) {
	    if (e->getSpouse() == nullptr && fatherOfMother != nullptr) {
	      if (oneFound && fatherOfMother->getLastName() == twoLast_ &&
		  e->getFirstName() == twoFirst_) {
		canMarry_ = false;
		return;
	      }
	      if (!oneFound && fatherOfMother->getLastName() == oneLast_ &&
		  e->getFirstName() == oneFirst_) {
		canMarry_ = false;
		return;
	      }
