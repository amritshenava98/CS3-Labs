// modified implementation basic hashmap (unordered container)
// Dan Fabian
// 2/28/2019
...
template <typename Key, typename Value, 
 ...
 void rehash(size_t n);            // sets num of buckets to n and rehashes
 private:
...
template <typename Key, typename Value, typename Compare, typename Hash>
  ...
  if (it == elems_[bucket].end()) {
    ...
    --it;
    return make_pair(&(*it), true); // return pair
  } 
  else 
...
template <typename Key, typename Value, typename Compare, typename Hash>
  ...
  if (it != elems_[bucket].end()) {    // the element exists, erase it
    auto deleteIt = it; // store iterator to be deleted
    ++it; // move forward iterator to return

    elems_[bucket].erase(deleteIt);
    --size_;
	
    // check if it was the last element in the bucket
    // move to the next bucket
    if (it ==  elems_[bucket].end()) {
      // move iterator forward past empty buckets
      ++bucket;
      while (elems_[bucket].empty() && bucket != elems_.size())
	++bucket;

      if (bucket == elems_.size()) // check if deleted the final element in the last bucket
	return make_pair(nullptr, true);
      
      else // move to the begining of the next bucket
...
template <typename Key, typename Value, typename Compare, typename Hash>
  ...
  if (found == nullptr) { // if key not found, create new element with empty value
    auto tmp = insert(make_pair(x, 
				Value())); // calling default constructor on Value
    found = tmp.first; // inefficient but avoids code duplication
  }
...
template <typename Key, typename Value, typename Compare, typename Hash>
  ...
  // check if n is larger than num of buckets now
  ...
  // copy old elems to a tmp
  auto tmpBuckets = elems_;

  // create a new list of elems
  elems_ = vector<list<Element>>(hash_.numBuckets());

  // for all buckets
  for (const auto& bucket: tmpBuckets) {
    // for all elements in each bucket
