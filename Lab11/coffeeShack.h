...
Drink* JuniorBarista::handleRequest(char choice, list<Drink*>& drink)
{
  ...
  DrinkType type;
  double price;
  if      (choice == 'l') { type = DrinkType::large; price = 3; }
  else if (choice == 'm') { type = DrinkType::medium; price = 2; }
  else                    { type = DrinkType::small; price = 1; }
  ...
  while (choice != 'f')
    {
      ...
      if      (choice == 's') drink.push_back(new Sprinkles(drink.back()));
      else if (choice == 'c') drink.push_back(new Caramel(drink.back()));
      else if (choice == 'i') drink.push_back(new Ice(drink.back()));
      else if (choice == 'n') return drink.back();
  ...
  while (choice != 'n')
    {
      ...
      else if (choice == 'f') drink.push_back(new MilkFoam(drink.back()));
      else if (choice == 'i') drink.push_back(new Ice(drink.back()));
...
void Customer::notify() const
{
  cout << "Customer " << name_ << " reports "
       << "order is finished for " << subject_->getCustomerServed()
       << '\n';
}
